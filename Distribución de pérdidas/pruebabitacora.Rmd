...
grafico2 <-
  ...
  labs(
    y = "Frecuencia",
    x = "Cuantil",
    title = "Gráfico 2: Frecuencia del estimado de pérdidas de acuerdo al cuantil 0.95",
...
library(scales)
#library(ggplot2)
df <- data.frame(Name=c("Masa cuantil menor a .95", "Masa cuantil mayor o igual a .95"), Amount=c(sum(perd[perd < out]),sum(perd[perd >= out])))



df$Amount <- factor(df$Amount)

grafico3<-ggplot(df, aes(x=Name, y=Amount)) + geom_bar(stat="identity",fill = c('blue', 'red'),
    width = 0.5,
    na.rm = TRUE
  ) + theme_bw() +
  labs(
...
library(xtable)
print.xtable(xtable(t2), file = "t1.txt")
