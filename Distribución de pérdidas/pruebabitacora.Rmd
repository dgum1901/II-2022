...
ggplot(p, aes(x = Umbral, y = Exceso)) + geom_line() + theme_bw() + 
  labs(title = "Gráfico 8: Función media de exceso, iniciando el .95",
       caption = "Fuente: Elaboración propia")
```
...
perd <- base$logEstimado
d1 <- quantile(perd, 0.95)
d<-seq(d1,max(perd),length.out=n)#MeanExcess(perd)
p <- tibble(Umbral = d, Exceso = e(d))
# d1 <- nrow(p) - which.max(diff(rev(p$Exceso))<0)
# d1 <- p$Umbral[d1]
grafico9 <-
  ggplot(p, aes(x = Umbral, y = Exceso)) + geom_line() + theme_bw() + 
  labs(title = "Gráfico 9: Función media de exceso log-pérdida",
       caption = "Fuente: Elaboración propia")
grafico9
```

Como se vio en clases, a partir de este umbral hay una función lineal decreciente, por lo que la opción que mejor puede ajustar los datos es una Weibul, porque hablamos de colar ligeras. Observe la siguiente grafica inutil

```{r}
d<-d[d>d1] # Lo que esta arriba del umbral
plot(d,e(d),type='line')
```
