...
```
Seobtiene la media para la ciudad más alta y la ciudad más baja
```{r}
caca1 <- (reorder(df$x, df$y, FUN = length))
caca2 <- as.numeric(caca1)

df <- as.tibble(rep(caca1,caca2))

ggplot(df, aes(x = value)) + geom_histogram()

length(data$price[data$city == 'Seattle'])
length(data$price[data$city == 'Sammamish'])
length(data$price[data$city == 'Shoreline'])
```
