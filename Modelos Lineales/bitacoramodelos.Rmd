...
p <-
  data %>% select(is.numeric) %>% cor(use = "pairwise.complete.obs", method = "spearman")
...
```{r}
df <-  data.frame('superficie habitable' = data$sqft_living, 'superficie construccion' = data$sqft_above)
ggplot(df, aes(superficie.habitable, superficie.construccion))+ geom_point()

```
Condicion con
```{r}
df <- data.frame(condicion = data$condition, logprecio = data$logprecio)


df$condicion <- as.factor(df$condicion)
ggplot(df, aes(x=condicion, y=logprecio)) + 
  geom_boxplot()
